...
| intercom | `/vdm:intercom` | Central cross-agent/cross-session message store (send / check / pickup) outside all repos, with an agent directory (identity + the names the user says) kept complete by a SessionStart check |
...
A `SessionStart` hook (`crystal-hydrate`) lists active workitems so the assistant Reads them before continuing. Non-canonical statuses (anything outside the 4-tier taxonomy after `status-aliases` resolution) surface as audit warnings in `list-open-crystals`, `crystal-hydrate`, and `crystal-cave` for triage. `crystal-cave` additionally reports **structural** canon on a separate axis (v2.19.0) — `⚠ Off-canon shape` for workitems missing required sections/keys/DL fields, and `ℹ Legacy schema` for `crystal-migrate` imports whose shape was never asserted; rows carry the same markers inline. The two axes stay separate because a workitem can have a flawless `status:` and a broken shape, or the reverse. The verdict comes from `crystal-lint.sh --summary` rather than being re-derived, so "off canon" means one thing everywhere. The design document for the suite is itself the first crystal in this repo — `docs/tasks/crystal-design/workitem.md` — serving as a worked example of the format.

### intercom
Central **cross-agent / cross-session message store**. Leave a task brief or note for another repo's agent — or for a future clean session of your own (the common "note to future me" case) — with `/vdm:intercom send <target> <slug>`; list and consume your inbox with `check` / `pickup`. Messages live in a **single machine-level store outside all repos** (`$VDM_INTERCOM_ROOT` → `~/.claude/vdm-plugins.json` `intercom.root` → default `~/.claude/vdm/intercom`), so no repo's history or `.gitignore` is touched. Routing is by **canonical identity from the git remote slug**, not the directory basename — a self-registering registry keeps dir-name / `owner/repo` aliases so any of a project's names resolve to the same inbox. Every message carries an explicit `from`/`to` envelope; `pickup --grow` promotes a brief into a workitem via `crystal-grow`. Checking the inbox is explicit (`/vdm:intercom check`) — an auto-reminder exists but is **off by default** (a message landing mid-session is usually meant for a *new* session; opt in with `/vdm:intercom on`). Supersedes the older per-repo `_outbox/` handoff pattern. **Agent directory (v2.21.0).** The registry also records the names the *user* says — `"vdm"`, `"intercom"`, `"the plugins repo"` — and a one-line description, because nothing derived from a remote URL will ever produce those; a sender addresses any identity / alias / name (`send vdm …`, `resolve "the intercom agent"`, `directory` to see everyone) and the brief lands in the one canonical inbox. An unresolved or ambiguous target is a **hard stop** with suggestions, not a fresh inbox nobody reads (`--first-contact` overrides). A `SessionStart` hook (`intercom-identity-check.sh`, on by default) registers the mechanical part of every repo deterministically and, until the human part (names + description) is present, tells the assistant to complete it before other work — asking the user once if the names cannot be inferred; once complete it prints one line: *you are `<identity>` — aka …*, so "what's your name?" has an answer. A second remote for the same slug (working clone vs marketplace clone) is confirmed with `register --same-project`; a different project sharing the slug gets its own `intercom.identity`. The negative scenario closes at the moment the user answers: `send "<hint>" <slug> --to <identity>` delivers there and records the hint as that agent's name in the same command, so it resolves directly next time; a `--first-contact` inbox that turns out to be addressed to you under a name you registered later is surfaced at session start and brought home with `claim`.
