...
# ---------------------------------------------------------------------------
# Scrub git's per-invocation environment BEFORE anything else runs.
#
# This harness is executed by `.githooks/pre-commit`, i.e. as a child of a
# running `git commit`. Git exports its own plumbing variables to hooks, and
# for a pathspec commit (`git commit -- <paths>`, the form git-guard-prepare
# hands out) `GIT_INDEX_FILE` points at a TEMPORARY INDEX inside the real
# `.git/` holding exactly the tree about to be committed.
#
# Those variables are inherited by every `git` command below — including the
...
# It was not reproducible because it needs gate 6 to actually fire, which only
# happens when a gate file is staged.
#
#   A test harness that runs inside a hook inherits the caller's git session.
#   Scrub it, or the harness writes into the very commit it is inspecting.
#
...
# FALSE-POSITIVE 4: the same substring rooted at an INSTALL directory is a
# user-time path, not a dev-tree one — it resolves on the user's machine and
# nowhere in this clone, the exact inverse of what the gate catches. Needed
# because guard/SKILL.md emits a git-hook snippet that runs in a plain shell,
# where ${CLAUDE_PLUGIN_ROOT} is undefined (the harness sets it for skills, not
# for git hooks), so the snippet must resolve the install path itself.
printf '\n  for c in "$HOME"/.claude/plugins/marketplaces/*/plugins/vdm-git/scripts/crystal-precommit-check.sh; do :; done\n' >> "$SKILL"
out=$(bash scripts/check-skill-paths.sh 2>&1); rc=$?
expect_exit "GREEN: install-rooted path is not a dev-tree leak" 0 "$rc"
restore

# RED 3: and the carve-out must stay narrow — the anchor has to PRECEDE the
# match on the same line. A bare dev-tree path on a line that merely mentions
# an install directory afterwards is still a leak. Without this the exemption
# would be a hole shaped like a sentence.
printf '\nRun plugins/vdm-git/scripts/crystal-precommit-check.sh; installs land under ~/.claude/plugins/.\n' >> "$SKILL"
out=$(bash scripts/check-skill-paths.sh 2>&1); rc=$?
expect_exit "RED: install dir mentioned AFTER the path is still a leak" 1 "$rc"
expect_says "RED: names the file" "$out" "docs-distill/SKILL.md"
restore
...
# FALSE-POSITIVE 4: a crystal legitimately holds reference artifacts under
# <slug>/references/ — intercom briefs, exported specs, transcripts. They carry
# whatever frontmatter their ORIGIN gave them: an intercom brief arrives with
# `status: pending`, which is not in the workitem taxonomy and never should be.
# They are not workitems and do not owe the canon.
#
# This is the exact false positive that shipped: the gate filtered on `*.md`
# under a crystal root, so every reference file was linted and reported a stack
# of violations. A linter that fires on legitimate files gets switched off —
# and then the real violations go too.
mkdir -p "docs/tasks/$canon_slug/references"
canon_write in-progress "$CANON_OK"
{
  printf -- '---\n'
  printf 'intercom: v1\nfrom: someagent\nto: thisrepo\n'
  printf 'slug: %s\nstatus: pending\n' "$canon_slug"
  printf -- '---\n\n# a brief, not a workitem\n\nbody\n'
} > "docs/tasks/$canon_slug/references/brief.md"
git add "docs/tasks/$canon_slug/workitem.md" "docs/tasks/$canon_slug/references/brief.md"
out=$(bash plugins/vdm/scripts/crystal-lint.sh --staged 2>&1); rc=$?
expect_exit "GREEN: references/ artifact is not a workitem" 0 "$rc"
expect_not_says "GREEN: reference file is not named" "$out" "references/brief.md"
restore

# FALSE-POSITIVE 5: the pair that proves the rule above is SCOPE, not blindness.
# Same directory, same broken shape — but named workitem.md, so it is in scope.
mkdir -p "docs/tasks/$canon_slug"
canon_write in-progress '## Назначение
x'
git add "docs/tasks/$canon_slug/workitem.md"
out=$(bash plugins/vdm/scripts/crystal-lint.sh --staged 2>&1); rc=$?
expect_exit "RED: workitem.md in the same crystal is still in scope" 1 "$rc"
restore

# FALSE-POSITIVE 6: the flat legacy form (<root>/<slug>.md, depth 1) stays in
# scope — dropping it while excluding references/ would be an easy over-correction.
canon_flat="docs/tasks/zz-flat-$$.md"
{
  printf -- '---\n'
  printf 'title: "flat fixture"\nslug: zz-flat\nstatus: in-progress\n'
  printf 'session-type: other\ncreated: 2026-08-22\nlast-updated: 2026-08-22\n'
  printf -- '---\n\n# flat fixture\n\n## Назначение\nx\n'
} > "$canon_flat"
git add -- "$canon_flat"
